...
errorEngine generates an incredibly tight, strictly-structured JSON `Payload` natively. You can completely bypass installing giant third-party log-shippers by merely supplying a custom `transports` array filled with simple async functions. errorEngine will execute your array of functions invisibly in the background every time a crash happens!

## Configuration & Usage

### Sending Errors to Slack or Discord

You can use the native Javascript `fetch()` command inside your transport array to beam errorEngine's curated JSON explanations to a chat webhook instantly:
...
Transports don't have to be network calls. You can use it to map errorEngine to custom file outputs.
...
When writing your transport function, it is critical to know what errorEngine is handing you. The `payload` object passed to your transport function follows this rigid schema:
