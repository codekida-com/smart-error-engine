...
errorEngine implements an incredible development feature utilizing Node.js's native `async_hooks` module.

When enabled, errorEngine essentially acts as a thread surveillance system. As asynchronous executions are spawned, it temporarily buffers their origin stacks in memory. If a thread unexpectedly fatally crashes, errorEngine will extract that thread's ID, match it against the buffered origins, and dynamically "stitch" the broken stack trace halves back together.
...
The `--- async boundary ---` is injected gracefully by errorEngine so you know exactly which original function (`triggerAsyncCrash`) scheduled the future payload that eventually threw an exception!
