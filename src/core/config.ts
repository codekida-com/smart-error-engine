...
export interface errorEngineOptions extends StyleOptions {
  mode?: 'auto' | 'dev' | 'prod';
...
export class ConfigManager {
  ...
  private options: errorEngineOptions = {
    mode: 'auto',
    ignore: [],
    format: 'auto' as any, 
    color: 'auto',
  ...
  private loadEnv() {
    if (typeof process !== 'undefined' && process.env) {
      if (process.env.ERROR_ENGINE_FORMAT) {
         this.options.format = process.env.ERROR_ENGINE_FORMAT as any;
      }
      if (process.env.ERROR_ENGINE_COLOR === 'false') this.options.color = false;
      if (process.env.ERROR_ENGINE_COLOR === 'true') this.options.color = true;
      if (process.env.ERROR_ENGINE_MODE) this.options.mode = process.env.ERROR_ENGINE_MODE as any;
  ...
  update(options: errorEngineOptions) {
