...
export function showOverlay(error: Error, insight: InsightCard, payload: any) {
    ...
    let overlay = document.getElementById('smart-error-engine-overlay');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'smart-error-engine-overlay';
    ...
    overlay.innerHTML = `
        <div style="background: #222; padding: 30px; border-radius: 8px; border-left: 5px solid #e00; box-shadow: 0 4px 20px rgba(0,0,0,0.5); max-width: 800px; margin: 0 auto;">
            ...
            <button onclick="document.getElementById('smart-error-engine-overlay').remove()" style="background: #444; color: white; border: none; padding: 8px 16px; cursor: pointer; border-radius: 4px; font-weight: bold;">Dismiss</button>
        </div>
    `;
}
