...
// 3. Env Var
process.env.ERROR_ENGINE_COLOR = 'false';
const envNoColorStyle = getStyle();
if (envNoColorStyle.red('test') === 'test') {
    console.log('SUCCESS: ERROR_ENGINE_COLOR=false disables ANSI');
} else {
    console.error('FAILURE: ERROR_ENGINE_COLOR=false did not disable ANSI');
    process.exit(1);
}
