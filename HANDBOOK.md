# 📖 The errorEngine Contributor & Architecture Handbook

Welcome to the internal mechanics of `@codekida/smart-error-engine`. If you are completely new to the codebase, want to understand how the diagnostics engine works, or need to debug/extend its functionality, this is your ultimate guide.
...
Before diving into files, it is crucial to understand the **Path of a Crash**. When an error occurs in an application using `@codekida/smart-error-engine`, this is the exact journey it takes securely through the engine:
...
2. **Context Enrichment**: `@codekida/smart-error-engine` pauses the crash to scrape the surrounding environment (What OS? What Node version? What HTTP request caused this? What were the last 15 UI clicks?).
...
4. **The AI Fallback**: If no static rules match _and_ AI is enabled, `@codekida/smart-error-engine` securely POSTs the trace to a local or cloud LLM.
...
This is the public API of the library. When a user runs `import { errorEngine } from '@codekida/smart-error-engine'`, this is what they are importing. It simply exports the `init()` function and the framework-specific adapters (Express, Koa, Fastify error handlers).

### `src/core/config.ts` (The Brain)

- **What it does:** A Singleton class that aggressively manages user configurations, environment variables (`process.env.ERROR_ENGINE_MODE`), and remote configuration fetching.
- **How to modify:** If you ever need to add a new option to `errorEngine.init({ ... })`, you must add it to the `errorEngineOptions` typescript interface here and set a default value in the `ConfigManager`.
...
- **Code Flow:**
  ...
  2.  If no match, it loops through errorEngine's built-in `rules` array.
...
- **How to modify:** If you want errorEngine to automatically detect if a user is running inside Docker or Kubernetes, you would add the environment scraping logic to this file's `globalContext`.

### `src/core/fetch.ts` (The Cross-Platform Network Bridge)

- **What it does:** Because `@codekida/smart-error-engine` strictly enforces a "Zero External Dependencies" rule, it cannot install `node-fetch`. This file provides a wildly clever native `makeRequest` wrapper. If the user is on a modern environment (Browser or Node 18+), it natively executes `fetch()`. If the user is on legacy Node 14 or 16, it dynamically falls back to the low-level C++ `require('http')`/`require('https')` core modules to reconstruct network requests perfectly. This ensures `@codekida/smart-error-engine` runs safely on older servers without crashing.
