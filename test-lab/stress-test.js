...
const interval = setInterval(() => {
    ...
    if (count % 5 === 0) {
        ...
        // To test @codekida/smart-error-engine hooks, we should throw/reject
        
        // We need to catch them to prevent process exit if we want the Loop to continue?
        // But @codekida/smart-error-engine enforces process.exit(1) on 'error'.
        ...
        // Wait, if @codekida/smart-error-engine exits on error, we can't really stress test "multiple errors" unless they are warnings or we disable exit.
